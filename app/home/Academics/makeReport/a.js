...
const [isVisible, setIsVisible] = useState(false)
   ...
   const fetchStudents = async() => {
    ...
    try {
      ...
      setData(dataa)
      setClassMarks(dataa)
      // setStudents(dataa.students?.sort((a, b) => b.total - a.total) || []);
      setClassWorkMark(dataa?.classWork || '');
      setExamMark(dataa.exam || '');
    ...
    const handleSetPercentage = async (newExamPercentage, newClassWorkPercentage) => {
      if (!data?._id) {
        Alert.alert('Invalid class ID for updating percentages.');
        return;
      }
      ...
      try {
        ...
        Alert.alert('Failed to update percentages. Please try again.');
      } finally {
...
<Text style={styles.inputHeader}>{'Set Marks'}</Text>
            ...
            <View style={styles.inputButtonsContainer}>
              ...
              <TouchableOpacity style={styles.buttons} onPress={handleSetPercentage}>
