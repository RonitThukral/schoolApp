...
const ExpenditureReport = () => {
  ...
  // Date picker handlers - Updated for custom date picker
                      ...
                      {expenditures.length > 0 ? (
                        expenditures.map((item, rowIndex) => (
                          <View key={rowIndex} style={[
                            ...
                            <Text style={styles.cell}>{(item.paymentMethod?.toLowerCase() ||(item.pay?.bank ? 'bank deposit' : 'cash'))}</Text>
                            <Text style={styles.cell}>{item?.pay?.bank || item.bank  || "N/A"}</Text>  
